...
function populateInstDepartmentSelect() {
  const instDepts = masterProdDepartments.filter(
    (d) => (d.mainDepartment || '').toLowerCase() === 'installation'
      && d.isActive !== false && d.isActive !== 0
  );
...
async function onInstProjectChange() {
  ...
  await Promise.all([_ensureWorkOrdersMaster(), _ensureCompletedWoNos()]);
  const projWosInst = masterWorkOrders.filter(w => (w.projectCode || '') === projectId);
  console.log('[INST WO]', projectId, projWosInst.map(w => ({ wo: w.workOrderNumber, dept: w.departmentName, parent: w.parentDepartmentName })));
  const masterList   = masterWorkOrders.filter(w => {
    ...
    const isInstDept = dept === 'installation' || parentDept === 'installation';
    return (w.projectCode || '') === projectId &&
      isInstDept &&
      !_completedWoNos.has((w.workOrderNumber || '').trim());
  });
  const woOpts = masterList.map(w => ({ value: w.workOrderNumber || '', label: w.workOrderNumber || '-' }));
...
async function onProdProjectChange() {
  ...
  await Promise.all([_ensureWorkOrdersMaster(), _ensureCompletedWoNos()]);
  const projWos = masterWorkOrders.filter(w => (w.projectCode || '') === projectId);
  console.log('[PROD WO]', projectId, projWos.map(w => ({ wo: w.workOrderNumber, dept: w.departmentName, parent: w.parentDepartmentName })));
  const masterList = masterWorkOrders.filter((w) => {
    const dept       = (w.departmentName       || '').toLowerCase();
    const parentDept = (w.parentDepartmentName || '').toLowerCase();
    const isProdDept = dept === 'production' || parentDept === 'production';
    return (w.projectCode || '') === projectId &&
      isProdDept &&
      !_completedWoNos.has((w.workOrderNumber || '').trim());
  });
