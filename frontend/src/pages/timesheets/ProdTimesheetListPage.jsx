...
export default function ProdTimesheetListPage() {
  ...
  const isApprover = permissions.some((p) => p.module === 'PROD' && p.canWrite && p.canReport);
  const canCreate  = usePermission('PROD', 'canCreate');
  const canWrite   = usePermission('PROD', 'canWrite');
  const canDelete  = usePermission('PROD', 'canDelete');
