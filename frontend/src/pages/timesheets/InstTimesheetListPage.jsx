...
export default function InstTimesheetListPage() {
  ...
  const { data: timesheets = [], isLoading } = useQuery({
    queryKey: ['inst-timesheets', filters],
    queryFn: () => api.get('/timesheets', { params: { type: 'INST', ...filters } }).then((r) => r.data),
  });
  ...
  const { mutate: deleteTs } = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['inst-timesheets'] }); toast('Timesheet deleted.', 'success'); },
  ...
  const { mutate: submitTs } = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['inst-timesheets'] }); toast('Submitted for approval.', 'success'); },
  ...
  const { mutate: actTs } = useMutation({
    ...
    onSuccess: (_, vars) => {
      ...
      toast(vars.action === 'approve' ? 'Approved.' : 'Rejected.', 'success');
    },
  ...
  const columns = [
    ...
    { key: 'entered_by_name',label: 'Timesheet Entry Person', sort: true },
    {
