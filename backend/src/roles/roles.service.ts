...
export class RolesService implements OnModuleInit {
  ...
  async findAll(): Promise<Role[]> {
    ...
    const res = await this.pool.request().query<Role>(`
      SELECT r.roleCode, r.roleName, r.deptScope, ISNULL(r.dataScope,'All') AS dataScope, r.status,
  ...
  async create(body: { roleName: string; deptScope?: string; dataScope?: string; status?: string }): Promise<Role> {
    ...
    try {
      await this.pool.request()
        ...
        .query(`
          INSERT INTO PSTsRoles (roleCode, roleName, deptScope, dataScope, status)
          VALUES (@roleCode, @roleName, @deptScope, @dataScope, @status)
        `);
    } catch (err: any) {
  ...
  async update(roleCode: string, body: { roleName?: string; deptScope?: string; dataScope?: string; status?: string }): Promise<Role> {
    ...
    await this.pool.request()
      ...
      .query(`
        UPDATE PSTsRoles
        SET    roleName = @roleName, deptScope = @deptScope, dataScope = @dataScope,
        ...
        await tx.request()
          ...
          .query(`
            ...
            WHEN NOT MATCHED THEN
              INSERT (roleCode, module, canCreate, canRead, canWrite, canDelete, canReport)
              VALUES (@roleCode, @module, @canCreate, @canRead, @canWrite, @canDelete, @canReport);
          `);
      }
      await tx.commit();
    } catch (err) {
